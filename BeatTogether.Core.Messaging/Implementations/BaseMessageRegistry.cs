...
namespace BeatTogether.Core.Messaging.Implementations.Registries
{
    public abstract class BaseMessageRegistry : IMessageRegistry
    {
        ...
        private readonly Dictionary<uint, MessageFactory> _factoryByIdLookup;

        public BaseMessageRegistry()
        {
            _typeByIdLookup = new Dictionary<uint, Type>();
            _idByTypeLookup = new Dictionary<Type, uint>();
            _factoryByIdLookup = new Dictionary<uint, MessageFactory>();
        }
